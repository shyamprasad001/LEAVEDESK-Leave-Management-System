...
router.get('/dashboard', isAuthenticated, async (req, res) => {
  try {
    ...
    const leaves = await LeaveRequest.find({ faculty: userId }).sort({ createdAt: -1 }).limit(5);
    const stats = {
      total: await LeaveRequest.countDocuments({ faculty: userId }),
      pending: await LeaveRequest.countDocuments({ faculty: userId, status: 'Pending' }),
      approved: await LeaveRequest.countDocuments({ faculty: userId, status: 'Approved' }),
      rejected: await LeaveRequest.countDocuments({ faculty: userId, status: 'Rejected' })
    };
    res.render('faculty/dashboard', { title: 'Faculty Dashboard', leaves, stats });
  } catch (err) {
    console.error(err);
...
router.get('/leave/:id', isAuthenticated, async (req, res) => {
  try {
    const leave = await LeaveRequest.findOne({ _id: req.params.id, faculty: req.session.user._id })
...
router.post('/leave/:id/cancel', isAuthenticated, async (req, res) => {
  try {
    const leave = await LeaveRequest.findOne({ _id: req.params.id, faculty: req.session.user._id, status: 'Pending' });
    if (!leave) {
